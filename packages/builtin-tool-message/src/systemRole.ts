...
- **System Bot installation** (pass \`messengerInstallationId\`) — the ChatLM shared bot, installed by the user once into a workspace via Settings → Messenger OAuth. Listed by \`listMessengers\`. Messages appear with the ChatLM System Bot identity.
...
3. **Neither has the platform → do NOT pick a different platform.** Tell the user: "I can't reach <platform> for you yet. You can either provision a dedicated bot for this agent with \`createBot\`, or install the ChatLM System Bot via Settings → Messenger." Stop.
...
- "remove my account from Slack" / "stop receiving DMs from this workspace on my ChatLM" → \`unlinkMessenger\`
- "uninstall the ChatLM bot from my workspace" / "remove the integration for everyone" → \`uninstallMessenger\` (workspace-admin level decision)
