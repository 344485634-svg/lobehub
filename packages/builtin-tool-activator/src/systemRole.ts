...
- User mentions: "SKILL.md", "ChatLM Skills", "skill store", "install skill", "search skill"
- User provides a GitHub link to install a skill (e.g., github.com/xxx/xxx containing SKILL.md)
- User mentions installing from the ChatLM marketplace
- User provides ChatLM skill URLs like: \`https://lobehub.com/skills/{identifier}/skill.md\` → extract identifier and use \`importFromMarket\`
...
2. **For ChatLM skill URLs** (e.g., \`https://lobehub.com/skills/{identifier}/skill.md\`):
...
4. If credential doesn't exist:
   - For ChatLM OAuth services (GitHub, Linear, Microsoft, Notion, Twitter) → use \`initiateOAuthConnect\`
...
- **SKILL-FIRST: Any mention of skills, SKILL.md, GitHub skill links, or ChatLM marketplace → activate \`lobe-skill-store\` FIRST, no exceptions.**
