 ...
 *   directory. ChatLM-managed credentials are deliberately NOT injected into
...
> = {
  'device':
    "Execution environment: the user's selected device, not a cloud sandbox. The skill archive is auto-extracted on the device and the command runs in the skill directory. ChatLM-managed credentials (e.g. `GITHUB_TOKEN`) are NOT injected.",
  'device-unrouted':
...
> = {
  'device':
    'A local device is routed: `execScript` runs skill scripts on the device (archive auto-extracted, cwd = skill directory); use `lobe-local-system` runCommand for other shell commands. ChatLM-managed credentials are not available on the device.',
  'device-unrouted':
