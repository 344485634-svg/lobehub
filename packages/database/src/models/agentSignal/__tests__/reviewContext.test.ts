...
describe('AgentSignalReviewContextModel', () => {
  describe('canAgentRunSelfIteration', () => {
     ...
     */
    it('allows inbox LM Studio even when virtual and selfIteration chat config is absent', async () => {
     ...
     */
    it('keeps non-LM Studio agents behind chatConfig.selfIteration.enabled', async () => {
      await serverDB.insert(users).values({ id: userId });
      await serverDB.insert(agents).values([
        {
