...
function buildNotifyProtocol(lhPath: string, topicId: string): string {
  return (
    `## Context: This task was dispatched by ChatLM\n\n` +
    `This conversation / task was sent to you by the **ChatLM platform** on behalf of a user. You are running as a background agent; the user is waiting for your response inside the ChatLM chat interface.\n\n` +
