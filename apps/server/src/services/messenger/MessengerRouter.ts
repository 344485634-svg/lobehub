 ...
 *
 * Account model: each `(ChatLM user, platform, tenant_id)` triple has at
 * most one row in `messenger_account_links`, so a single ChatLM user can
...
export class MessengerRouter {
  ...
  private registerHandlers(
    ...
    // listing reviewers also test the `/invite @ChatLM` entry point, so the
  ...
  private buildCommands(): MessengerCommand[] {
    return [
      {
        description: 'Bind your account to ChatLM',
        handler: async (ctx) => {
          ...
          if (ctx.link) {
            await ctx.reply(
              'Your account is already linked to ChatLM. Send /agents to switch the active agent, or /new to start a fresh conversation.',
            );
            return;
          }
          ...
          if (!ctx.isDM && !canEphemeralInChannel) {
            await ctx.reply('Check your DM with ChatLM for the link button.');
          }
        },
        name: 'start',
      },
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            ...
            await ctx.reply('Open your direct message with the ChatLM bot and send `/new` there.');
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          ...
          if (!ctx.thread) {
            await ctx.reply('Open your direct message with the ChatLM bot and send `/stop` there.');
            return;
          }
      ...
      },
      {
        ...
        handler: async (ctx) => {
          // Feedback is tied to an ChatLM account so the team can follow up;
  ...
  private async runAgentsCommand(ctx: MessengerCommandContext): Promise<void> {
    ...
    if (userAgents.length === 0) {
      await ctx.reply('You have no agents yet. Create one in ChatLM, then come back to /agents.');
      return;
    }
   ...
   * which ChatLM context a conversation runs in is the active agent's scope;
  ...
  private scopeSwitchedText(scopeName: string, defaultAgentTitle?: string): string {
    return defaultAgentTitle
      ...
      : `Switched to ${scopeName}. No agents here yet — create one in ChatLM, then /agents.`;
  }
  ...
  private async handleAppHomeOpened(
    ...
    try {
      ...
      const text = activeAgentName
        ? `Welcome to ChatLM! Your active agent is *${activeAgentName}*. Send a message to chat, or use \`/agents\` to switch.`
        : 'Welcome to ChatLM! Send `/agents` to pick an active agent and start chatting.';
   ...
   * itself joins a channel (via `/invite @ChatLM` or being added through the
  ...
  private async handleChannelJoin(
    ...
    const text = [
      ":wave: Hi, I'm *ChatLM* — your AI agent on Slack.",
      '',
      '• Mention me with `@ChatLM <your question>` to chat in this channel.',
      '• First time? Send me a *direct message* to link your ChatLM account.',
  ...
  private async dispatchToAgent(
    ...
    // ChatLM user; the dispatch only fires for the linked sender. So
