...
export function generateRootManPage(program: Command, version: string) {
  ...
  const lines = [
    ...
    '.SH NAME',
    `lh \\- ${escapeRoff(program.description() || 'ChatLM CLI')}`,
    '.SH SYNOPSIS',
    ...formatSynopsisLines(),
    '.SH DESCRIPTION',
    escapeRoff(
      `${program.name()} is the command-line interface for ChatLM. It provides authentication, device gateway connectivity, content generation, resource search, and management commands for agents, files, models, providers, plugins, knowledge bases, threads, topics, and related resources.`,
    ),
    '.PP',
